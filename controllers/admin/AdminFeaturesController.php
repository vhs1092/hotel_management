...
class AdminFeaturesControllerCore extends AdminController
{
	...
	public function initPageHeaderToolbar()
	{
		if (empty($this->display))
		{
			...
			/*$this->page_header_toolbar_btn['new_feature_value'] = array(
				...
				'icon' => 'process-icon-new'
			);*//*by webkul feature value will be added from add feature buttun in form of image*/
		}
	...
	public function uploadFeatureImage($image, $feature_img_path)
	{
		if (isset($image))
		{
			if ($image['size'] > 0)
				ImageManager::resize($image['tmp_name'], $feature_img_path);
		}
	}

	// validate feature image by webkul
	public function validAddFeatureImage($image)
	{
		if ($image['size'] > 0)
		{			
			if ($image['tmp_name'] != "")
			{
				if(!ImageManager::isCorrectImageFileExt($image['name']))
				  	$this->errors[] = Tools::displayError('<strong>'.$_FILES['logo']['name'].'</strong> : Image format not recognized, allowed formats are: .gif, .jpg, .png', false);
			}
		}
		else
			return true;
